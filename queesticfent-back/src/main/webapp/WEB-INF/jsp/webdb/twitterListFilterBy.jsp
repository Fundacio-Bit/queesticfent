...
<un:useConstants var="TwitterFields" className="org.fundaciobit.queesticfent.model.fields.TwitterFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.ID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="twitter.id" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.USERNAME)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="twitter.username" var="username" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.COMENTARIO)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="twitter.comentario" var="comentario" />
              <fmt:message key="genapp.form.searchby" var="cercapercomentario" >                
                 <fmt:param value="${comentario}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${comentario}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercapercomentario}" path="comentario" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.FECHA)}">
            <%-- FILTRE DATE --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="twitter.fecha" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
            <div class="form-group"  style="margin-bottom: 0px;" >
                <div class="input-group date" id="fechaDesde" data-target-input="nearest">
                      <form:input  cssClass="form-control datetimepicker-input"  data-target="#fechaDesde" path="fechaDesde" />
                    <c:if test="${!false}" >
        ...
        <script type="text/javascript">
            $(function () {
              ...
              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>
            <div class="form-group"  style="margin-bottom: 0px;" >
                <div class="input-group date" id="fechaFins" data-target-input="nearest">
                      <form:input  cssClass="form-control datetimepicker-input"  data-target="#fechaFins" path="fechaFins" />
                    <c:if test="${!false}" >
                    <div class="input-group-append"  data-target="#fechaFins"  data-toggle="datetimepicker">
                        <div class="input-group-text"><i class="fa fa-calendar"></i></div>
                    </div>
                    </c:if>
                </div>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.USER_ID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="twitter.user_id" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="user_idDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="user_idFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TwitterFields.IP)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="twitter.ip" var="ip" />
